      ...
      <div className="p-4 rounded-xl space-y-4" style={{ background: "var(--surface)" }}>
        <div className="flex gap-2">
          <input
            ...
            onKeyDown={(e) => e.key === "Enter" && email.trim() && addMutation.mutate(email.trim())}
            placeholder="Email member..."
            className="flex-1 h-11 px-4 rounded-lg text-[14px]"
            style={{ border: "1px solid var(--border-visible)", background: "var(--black)", color: "var(--text-primary)", outline: "none" }}
          />
          <button
        ...
        {isLoading ? (
          ...
          <div className="text-center py-8">
            <p className="text-[13px]" style={{ color: "var(--text-disabled)" }}>Belum ada member.</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr style={{ background: "var(--surface-raised)", borderBottom: "1px solid var(--border)" }}>
                  <th className="p-3 text-[12px] font-medium" style={{ color: "var(--text-secondary)" }}>Email</th>
                  <th className="p-3 text-[12px] font-medium text-center w-[60px]" style={{ color: "var(--text-secondary)" }}>Aksi</th>
                </tr>
              </thead>
              <tbody>
                {members.map((m: any, idx: number) => (
                  <tr key={m.id} style={{ borderBottom: idx < members.length - 1 ? "1px solid var(--border)" : "none", background: "var(--black)" }}>
                    <td className="p-3">
                      <span className="text-[13px]" style={{ color: "var(--text-primary)" }}>{m.member_email}</span>
                    </td>
